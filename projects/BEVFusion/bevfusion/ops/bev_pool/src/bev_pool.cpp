...
void bev_pool_xpu(int b, int d, int h, int w, int n, int c, int n_intervals,
                  const float* x, const int* geom_feats, const int* interval_starts,
                  const int* interval_lengths, float* out);

void bev_pool_grad_xpu(int b, int d, int h, int w, int n, int c, int n_intervals,
                       const float* out_grad, const int* geom_feats,
                       const int* interval_starts, const int* interval_lengths, float* x_grad);
#endif
...
at::Tensor bev_pool_forward(
  ...
  const c10::OptionalDeviceGuard device_guard(_x.device());
  ...
  if (_x.device().type() == c10::DeviceType::CUDA) {
#ifdef WITH_CUDA
    bev_pool(
...
#else
    TORCH_CHECK(false, "bev_pool built without CUDA support");
#endif
  } else if (_x.device().type() == c10::DeviceType::XPU) {
#ifdef WITH_XPU
    bev_pool_xpu(
      b, d, h, w, n, c, n_intervals, x,
      geom_feats, interval_starts, interval_lengths, out
...
#else
    TORCH_CHECK(false, "bev_pool built without XPU support");
#endif
  } else {
    TORCH_CHECK(false, "Unsupported device type for bev_pool_forward");
  }
  return _out;
}
...
at::Tensor bev_pool_backward(
  ...
  int n_intervals = _interval_lengths.size(0);
  const c10::OptionalDeviceGuard device_guard(_out_grad.device());
  ...
  if (_out_grad.device().type() == c10::DeviceType::CUDA) {
#ifdef WITH_CUDA
    bev_pool_grad(
    ...
    TORCH_CHECK(false, "bev_pool built without CUDA support");
#endif
  } else if (_out_grad.device().type() == c10::DeviceType::XPU) {
#ifdef WITH_XPU
    bev_pool_grad_xpu(
      b, d, h, w, n, c, n_intervals, out_grad,
      geom_feats, interval_starts, interval_lengths, x_grad
    ...
    TORCH_CHECK(false, "Unsupported device type for bev_pool_backward");
  }
